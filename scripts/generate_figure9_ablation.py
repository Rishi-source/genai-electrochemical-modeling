...
ax1_twin.set_ylabel('RMSE (mV)', fontsize=12, fontweight='bold', color='#3498db')
...
line2 = ax2_twin.plot(x_pos, effort_reduction, 's-',
                     label='Human Effort Reduction', color='#2ecc71',
                     linewidth=3, markersize=10, markeredgecolor='black', markeredgewidth=1.5)
...
ax2_twin.set_ylabel('Human Effort Reduction (%)', fontsize=12, fontweight='bold', color='#2ecc71')
...
* Violations: {violations[0]:.1f}% -> {violations[-1]:.1f}% ({(violations[0]-violations[-1])/violations[0]*100:.0f}% down)
* Compile errors: {compile_errors[0]:.1f}% -> {compile_errors[-1]:.1f}% ({(compile_errors[0]-compile_errors[-1])/compile_errors[0]*100:.0f}% down)
