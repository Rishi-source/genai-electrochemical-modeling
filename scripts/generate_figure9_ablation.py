"""
Generate Figure 9: Ablation Study Results
Shows progressive improvement with each component added.
"""
...
# Plot 1: Constraint violations (bar) + RMSE (line)
...
ax1.set_ylabel('Constraint Violation Rate (%)', fontsize=12, fontweight='bold', color='#e74c3c')
ax1_twin.set_ylabel('RMSE (mV)', fontsize=12, fontweight='bold', color='#2ecc71')
...
# Plot 2: Compile errors (bar) + Human effort reduction (line)
# Bar chart for compile errors
...
# Line plot for effort reduction on secondary axis
ax2_twin = ax2.twinx()
line2 = ax2_twin.plot(x_pos, effort_reduction, 's-',
                     label='Human Effort Reduction', color='#3498db',
                     linewidth=3, markersize=10, markeredgecolor='black', markeredgewidth=1.5)

# Labels and formatting
ax2.set_xlabel('Configuration', fontsize=13, fontweight='bold')
ax2.set_ylabel('Compile Error Rate (%)', fontsize=12, fontweight='bold', color='#f39c12')
ax2_twin.set_ylabel('Human Effort Reduction (%)', fontsize=12, fontweight='bold', color='#3498db')
